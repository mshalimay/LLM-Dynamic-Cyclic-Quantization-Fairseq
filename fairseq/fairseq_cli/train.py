...
def main(cfg: FairseqConfig) -> None:
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        # train for one epoch
        valid_losses, should_stop = train(cfg, trainer, task, epoch_itr, update_total_iters)
        update_total_iters = False
        if should_stop:
            break
...
def train(
    cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, update_total_iters
    ...
    # NOTE compute total number of iterations for the entire training 
    # to compute cyclic precision parameter `cyclic_period`
    if update_total_iters:
        total_num_itrs = itr.total_num_itrs * cfg.optimization.max_epoch
        trainer.set_total_iters(total_num_itrs)
        print(f"\n\n\n\nTotal number of iterations: {total_num_itrs}")

...
#NOTE Globals for the cyclic precision scheduler config
script_directory = os.path.dirname(os.path.realpath(__file__))
cpt_cfg_path = os.path.join(script_directory, 'cfg_cpt.json')

class CyclicPrecisionScheduler:
    def __init__(self, wbit_min, wbit_max, inbit_min, inbit_max, num_cyclic_period, 
                 start_iter=0, print_every=1):
        self.wbit_min = wbit_min
        self.wbit_max = wbit_max
        self.inbit_min = inbit_min
        self.inbit_max = inbit_max
        self.print_every = print_every
        self.num_cyclic_period = num_cyclic_period
        
        # initialize bits to min values
        
        self.wbit = wbit_min
        self.prev_wbit = wbit_min
        self.inbit = inbit_min
        self.prev_inbit = inbit_min
        self.iter = start_iter

        # is cyclic?
        self.wcyclic = False
        if wbit_min is None or wbit_max is None:
            self.wcyclic, self.incyclic = False
        elif wbit_min < wbit_max:
            self.wcyclic, self.incyclic = True, True

        self.incyclic = False
        if inbit_min is None or inbit_max is None:
            self.wcyclic, self.incyclic = False, False
        elif inbit_min < inbit_max:
            self.incyclic = True

        
    def step(self):
        if self.wcyclic:
            self.prev_wbit = self.wbit
            self.wbit = np.rint(self.wbit_min +
                            0.5 * (self.wbit_max  - self.wbit_min) *
                            (1 + np.cos(np.pi * ((self.iter % self.cyclic_period) / self.cyclic_period) + np.pi)))

        if self.incyclic:
            self.prev_inbit = self.inbit
            self.inbit = np.rint(self.inbit_min +
                            0.5 * (self.inbit_max  - self.inbit_min) *
                            (1 + np.cos(np.pi * ((self.iter % self.cyclic_period) / self.cyclic_period) + np.pi)))
        self.iter += 1
        
    def update_precision(self, module):
        if self.wcyclic or self.incyclic:
            # print(self.prev_wbit, self.wbit)
            if self.prev_inbit != self.inbit or self.prev_wbit != self.wbit:
                quantize.update_nbits(module, self.wbit, self.inbit)
                if self.iter % self.print_every == 0:
                    print(f'Iter [{self.iter}] w_bits = {self.wbit} in_bits = {self.inbit} cyclic precision')
...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        #NOTE instantiate cyclic precision scheduler
        # load config from the config file
        import json
        with open(cpt_cfg_path, 'r') as file:
            cfg_cpt = json.load(file)

        print("\n\n Cyclic Precision Scheduler Config: ", cfg_cpt)
        
        self.cpt = CyclicPrecisionScheduler(
            wbit_max=cfg_cpt['wbit_max'], wbit_min=cfg_cpt['wbit_min'], 
            inbit_max=cfg_cpt['inbit_max'], inbit_min=cfg_cpt['inbit_min'], 
            num_cyclic_period=cfg_cpt['num_cyclic_period'], 
            start_iter=0)
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            self.cpt.cyclic_period = self.total_iters // self.cpt.num_cyclic_period                        
            #NOTE adjust precision
            self.cpt.step()
            self.cpt.update_precision(self.model)
